...
export const optionError = ref<boolean[]>([]);
...
export const validataQuestion = (text: string, index: number) => {
  validataList(quesError, isOptionError, optionContentList, isValidataOption, text, index);
};
...
export const validataOptions = (text: string, index: number) => {
  validataList(optionError, isOptionError, quesContentList, isValidataQues, text, index);
};
